...
Support: http://pop-up.com/support

Licence:
pop-up JS is licensed under a Creative Commons Attribution-NonCommercial 2.5
...
// See http://pop-up.com/ref for examples of settings  
graphicsDir : 'pop-up/graphics/',
...
creditsHref : 'http://vikjavev.no/pop-up/',
...
wrapperClassName : 'pop-up-wrapper', // for enhanced css-control
...
{	
	...
	var el = e.target;
	while (el.parentNode
		&& !(/pop-up-(image|move|html|resize)/.test(el.className)))
	{
		el = el.parentNode;
	}
	...
	if (exp && e.type == 'mousedown') {
		...
		var match = el.className.match(/pop-up-(image|move|resize)/);
		if (match) {
			...
			if (/pop-up-(image|html)-blur/.test(exp.content.className)) {
				exp.focus();
				hs.hasFocused = true;
			}
			return false;
		...
		} else if (/pop-up-image-blur/.test(el.className)) {
		...
		(function(){
			...
			setTimeout ( function() {
				if (up && pI == 1) {
					...
					exp.a.className += ' pop-up-active-anchor';
				}
				exp.setSize(size);
			}, t);				
		})();
	...
	for (var i = 0; i < types.length; i++) {
		...
		if (!this[type] && !s) {
			...
			while (next && !hs.isHsAnchor(next)) {
				if ((new RegExp('pop-up-'+ type)).test(next.className || null)) {
		...
		if (!this[type] && s) this[type] = hs.createElement('div', 
				{ className: 'pop-up-'+ type, innerHTML: s } );
...
focus : function() {
	...
	for (var i = 0; i < hs.expanders.length; i++) {
		if (hs.expanders[i] && i == hs.focusKey) {
			...
			blurExp.content.className += ' pop-up-'+ blurExp.contentType +'-blur';
	...
	this.content.className = 'pop-up-'+ this.contentType;
...
getOverlays : function() {	
	...
	if (this.heading && this.dragByHeading) this.heading.className += ' pop-up-move';
...
afterClose : function () {
	this.a.className = this.a.className.replace('pop-up-active-anchor', '');
